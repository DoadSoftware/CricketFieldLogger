...
{
    public partial class Page1Frm : Form
    {
        string which_btn = "", style = "";
        public int player_id = -1;
        public FieldersData fieldersData = new FieldersData(false, "RHB", new List<FieldersFormation>());
        ...
        private void FielderLbl_Move(object sender, EventArgs e)
        {
            ...
            System.Diagnostics.Debug.WriteLine(fieldersData.Style);
            if (checkBox2.Checked)
            {
                 fieldersData.Checkbox = true;
                File.WriteAllText(FieldLoggerConstants.Cricket_Directory + FieldLoggerConstants.Field_Formation1_File,
...
*/
        ...
        {
            if (File.Exists(FieldLoggerConstants.Cricket_Directory +
                FieldLoggerConstants.Field_Formation1_File))
            {
                if (fieldersData.Style == "LHB")
                {
                    ...
                    fieldersData.Style = "RHB";
                    label3.Text = "RHB";
                    FieldLoggerPicBox.BackgroundImage = Image.FromFile(@"" + FieldLoggerConstants.Cricket_Directory + "Images\\wagon_rhb.jpeg");
                }
                else
                {
                    fieldersData.Style = "LHB";
                    label3.Text = "LHB";
                    FieldLoggerPicBox.BackgroundImage = Image.FromFile(@"" + FieldLoggerConstants.Cricket_Directory + "Images\\wagon_lhb.jpeg");
                }
                // Serialize and save the current fieldersData
                File.WriteAllText(FieldLoggerConstants.Cricket_Directory +
                    FieldLoggerConstants.Field_Formation1_File, JsonConvert.SerializeObject(fieldersData));

                // Initialize the fieldersData from the saved JSON
                using (StreamReader r = new StreamReader(FieldLoggerConstants.Cricket_Directory  + FieldLoggerConstants.Field_Formation1_File))
                {
                ...
                if (fieldersData != null)
                {
                    for (int i = 0; i < FieldLoggerConstants.Number_Of_Fielders; i++)
                    {
                        ...
                        File.WriteAllText(FieldLoggerConstants.Cricket_Directory +
                            FieldLoggerConstants.Field_Formation1_File, updatedJsonData);
                    }
                }
            }
        }
        ...
        private void checkbox2_btn_Click(object sender, EventArgs e)
        {
            ...
            FieldersData existingData = JsonConvert.DeserializeObject<FieldersData>(
                File.ReadAllText(FieldLoggerConstants.Cricket_Directory +  FieldLoggerConstants.Field_Formation1_File) );
            ...
            else
            {
             ...
             File.WriteAllText(FieldLoggerConstants.Cricket_Directory + FieldLoggerConstants.Field_Formation1_File, JsonConvert.SerializeObject(existingData));
            ...
            string filePath = @"" + FieldLoggerConstants.Cricket_Directory + "Fielder_Text\\FieldPlotter.txt"; // Replace this with the desired file path
                                                                                                               // string FieldLoggerConstants.filePath  = @"C:\\Sports\\Cricket\\FieldPlotter.txt";
            if (File.Exists(filePath))
            {
                ...
                if (lines.Length >= 2)
                {
                    ...
                    if (string.IsNullOrWhiteSpace(lines[0]) && string.IsNullOrWhiteSpace(lines[1]))
                    {
                        if (checkBox2.Checked)
                        {
                            lines[0] = "FielderFormation_1.JSON";
                            lines[1] = "true";
                        }
                        else
                        {
                ...
                else
                {
                    ...
                    newLines.Add("FielderFormation_1.JSON");
                    if (checkBox2.Checked)
                    {
                        newLines.Add("true");
                    }
            ...
            {            // Writing content to the text file
                using (StreamWriter outputFile = new StreamWriter(filePath))
                {
                    if (checkBox2.Checked)
                    {
                        outputFile.WriteLine("FielderFormation_1.JSON");
        ...
        {
            player_id = id;
            if (File.Exists(FieldLoggerConstants.Cricket_Directory + FieldLoggerConstants.Field_Formation1_File))
            {
                FieldersData existingData = JsonConvert.DeserializeObject<FieldersData>(File.ReadAllText(FieldLoggerConstants.Cricket_Directory + FieldLoggerConstants.Field_Formation1_File));
        ...
        private void Move_Player_On_Click(object sender, MouseEventArgs e)
        {
            if (player_id > -1)
            {
                ...
                if (File.Exists(filePath))
                {
                    ...
                    string l6 = "After_player_Top\t" + e.X;
                    string l7 = "After_Player_Left\t" + e.Y;

                    if (lines.Length >= 7)
                    {
                        // If the lines at specified indexes are empty or if the file is empty, write the lines
                        if (string.IsNullOrWhiteSpace(lines[2]) && string.IsNullOrWhiteSpace(lines[3]) && string.IsNullOrWhiteSpace(lines[4]) && string.IsNullOrWhiteSpace(lines[5]) && string.IsNullOrWhiteSpace(lines[6]))
                        {
                            lines[2] = l3;
                            lines[3] = l4;
                            lines[4] = l5;
        ...
        private void checkbox_btn_Click(object sender, EventArgs e)
        {
            if (File.Exists(FieldLoggerConstants.Cricket_Directory + FieldLoggerConstants.Field_Formation1_File))
            {
                string existingData = File.ReadAllText(FieldLoggerConstants.Cricket_Directory + FieldLoggerConstants.Field_Formation1_File);
                ...
                if (!checkBox1.Checked)
                {
                    for (int i = 0; i < FieldLoggerConstants.Number_Of_Fielders; i++)
                    {
                        ...
                        if (existingPlayer != null)
                        {
                            ...
                            // You might need to update any other fields for the non-highlighted players
                        }
                    }
                    ...
                    else
                    {
                        for (int i = 0; i < FieldLoggerConstants.Number_Of_Fielders; i++)
                        {
                            for (int j = 0; j < substrings.Length; j++)
                            {
                                ...
                                {
                                    int index = parsedValue - 1;
                                    FielderLbl[index].BackColor = Color.DarkRed;
                                    FielderLbl[index].ForeColor = Color.White;
                ...
                using (StreamWriter file = new StreamWriter(FieldLoggerConstants.Cricket_Directory + FieldLoggerConstants.Field_Formation1_File, false))
                {
        ...
        void SetValue(string value)
        {
            ...
            // Update the BowlingEnd value if it has changed
            ...
            if (File.Exists(filePath))
            {
                try
                {
                    ...
                    // Update the location of the label based on the BowlingEnd value
                    for (int i = 0; i < FieldLoggerConstants.Number_Of_Fielders; i++)
                    {
                        try
                        {
                            // Retrieve locations from the JSON object
                            int leftLocation = Int16.Parse((string)obj["fielders"][i]["leftLocation"]);
                            int topLocation = Int16.Parse((string)obj["fielders"][i]["topLocation"]);

                            // Update label locations based on BowlingEnd value
                            if (fieldersData.BowlingEnd == "Far")
                            {
                        ...
                        {
                            MessageBox.Show($"Format error for fielder {i}: {ex.Message}");
                        }
                        catch (Exception ex)
                        {
                            MessageBox.Show($"Error for fielder {i}: {ex.Message}");
                        }
                    }
                }
                catch (Exception ex)
